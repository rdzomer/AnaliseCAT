...
const PautaCatPage: React.FC = () => {
  ...
  const filteredPleitos = useMemo(() => {
    const filtered = pleitos.filter(p => {
      ...
      return matchesSearchTerm && matchesNCM && matchesStatus && matchesResponsavel && matchesPleiteante && matchesSessao && matchesPauta && matchesCgimFilter;
    });
  ...
  }, [pleitos, searchTerm, filterNCM, filterStatus, filterResponsavel, filterPleiteante, filterSessao, filterPauta, onlyCgimNcms, cgimNcmList]);
      ...
      <div className="bg-white shadow-lg rounded-xl overflow-x-auto">
        <table className="w-full min-w-max text-sm text-left text-gray-700">
          <thead className="text-xs text-gray-700 uppercase bg-gray-100">
            <tr>
              {['NCM', 'Produto', 'Pauta de Origem', 'Pleiteante', 'Tipo Alteração', 'Responsável', 'Status', 'Ações'].map(header => (
